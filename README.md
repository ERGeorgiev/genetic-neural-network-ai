...
```
Genetika/                  Core library
  Genetic/                 Genetic algorithm (Gene, Genome, Selection, Crossover, Mutation)
  Neural/                  Neural network (NeuralNetwork, NeuronLayer, Neuron, Weight)
  Interfaces/              IEntity<T> and ITablePrint<T>
  GenetikaManager.cs       Base class for running evolution simulations
  GenetikaParameters.cs    Configuration (population, rates, strategy types)

Genetika.Examples/         Example applications
  Velocity/                Learn to reach a target velocity
  Inertion/                Motion with inertia
  InertionDouble/          Complex inertia scenarios
  Translation/             Positional control
  VelocityWithNoise/       Learning with noisy signals
  Paint/                   Evolve neural networks to create paintings
  ImageRecognition/        Image classification

Genetika.Tests/            Unit tests (MSTest)
paintings/                 Sample AI-generated artwork
```
...
[MIT](LICENSE) -- Copyright (c) 2026 Eduard Georgiev
